...
// just use `Apply` or `MaybeApply`, which accept a
// spec string and a string to format. `Apply` returns an error
// if any bit of the spec string failed to match and bails as soon as a
// failure occurs, while `MaybeApply` simply returns the string
...
// `bold+green`. Additionally, colors can be specified as
// background rather than foreground colors by appending `:bg`
...
// spaces: 3/4-bit[0] (`Colors8`), 8-bit (`Colors256`),
// and 24-bit true-color (`Colors24bit`).
//
...
//
// `Attributes` and all three color spaces above, as implementers
// of `Collection`, implement a `Find` method which
// returns a `Formatter`, if one is found, and an error if one is
// not. For `Attributes` and `Colors8`, the search
// term is just a name ("bold", "green", etc), but `Colors256`
// and `Colors24bit`, you have more leeway. For the former,
...
// `Colors256` for, say, "#123512", you won't find it, despite that
// being a valid hex code. For that, use `Colors24bit`, which will
